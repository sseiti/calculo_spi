...
function createTables(){
    ...
    var query = 'CREATE TABLE IF NOT EXISTS calculo_spi_cfg(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, porcentagem REAL NOT NULL, valor REAL NOT NULL);';
    try {
        localDB.transaction(function(transaction){
            transaction.executeSql(query, [], nullDataHandler, errorHandler);
            //updateStatus("Tabela 'calculo_spi_cfg' status: OK.");
        });
    } 
    catch (e) {
        updateStatus("Erro: Data base 'calculo_spi_cfg' não criada " + e + ".");
        return;
    }    
    onSelectCfg();
}
